...
pub fn reviewer_v0_manifest() -> &'static CapabilityManifest {
    REVIEWER_V0_MANIFEST.get_or_init(|| {
        build_reviewer_v0_manifest().expect("reviewer v0 manifest should be valid")
    })
}
...
/// - [INV-TCK-00317-002] Write, Execute, Network, Inference are NOT in allowlist
...
mod tests {
    ...
    fn test_reviewer_v0_manifest_no_expiration() {
        let manifest = reviewer_v0_manifest();
        assert_eq!(manifest.expires_at, 0, "Canonical manifest should not expire");
    ...
    fn test_dynamic_manifest_has_timestamp() {
        ...
        assert!(manifest.created_at > 0, "Dynamic manifest should have timestamp");
